  ...
  onResizeGesture:function (gesture) {
    ...
    if (self.notifyLoc === 0) {
      ...
      if (temX > 200 && !chatAIUtils.isIOS()) {
        ...
        // MNUtil.showHUD("Release to change location")
      }else{
        self.screenButton.setImageForState(chatAIConfig.closeImage,0)
      }
    }else{
 ...
 * @this {notificationController}
 */
notificationController.prototype.customAsk = async function(question,noteId = undefined,title="Toolbar") {
try {
  this.noteid = noteId
  this.token = []
  this.func = []
  ...
  await this.beginNotification(title)
  this.question = question
...
try {
  ...
  MNUtil.animate(()=>{
    ...
    this.onAnimate = false
    this.screenButton.setImageForState(chatAIConfig.closeImage,0)
  })
} catch (error) {
...
try {
    ...
    switch (action) {
      ...
      case "reply":
        ...
        // return
        let userInputRes = await MNUtil.userInput("MN ChatAI", "Reply to the last message\n\n请输入回复内容", ["Cancel / 取消","Yes / 是","No / 不是","Continue / 继续","Exit / 退出","Update Memory / 更新记忆","Organize Memory / 整理记忆","Confirm / 确认"])
        switch (userInputRes.button) {
          ...
          case 4:
            ...
            return
          case 5:
            this.showHUD("💬 Reply: Update Memory")
            this.continueAsk("Update Memory")
            return
          case 6:
            this.showHUD("💬 Reply: Organize Memory")
            this.continueAsk("Organize Memory")
            return
          case 7:
