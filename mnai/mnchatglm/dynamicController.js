...
var dynamicController = JSB.defineClass(
  "dynamicController : UIViewController <NSURLConnectionDelegate,UIWebViewDelegate>",
  {
    ...
    viewWillLayoutSubviews: function () {
      ...
      self.historyButton.frame = MNUtil.genFrame(260, 75, 35, 35);
      ...
      self.settingButton.frame = MNUtil.genFrame(
        265,
        self.view.frame.height - 35,
        30,
        30,
      );
        ...
        switch (config.host) {
          ...
          case "sendMessage":
            ...
            if (content.trim()) {
              ...
              content = chatAIConfig.getLatestHistory();
    ...
    addContext: async function (params) {
      ...
      try {
        ...
        if ("imageData" in info) {
          ...
          chatAIUtils.sideOutputController.addToInput(
            userInput,
            info.imageData,
          );
        } else {
          ...
          if (note) {
            ...
            if (imageData) {
              chatAINetwork.getTextOCR(imageData).then(() => {
                self.OCREnhanced.backgroundColor = MNUtil.hexColorAlpha(
                  "#e06c75",
                  0.8,
                );
              });
    ...
    toggleVisionMode: async function (params) {
      if (chatAIUtils.visionMode) {
        ...
        self.visionButton.backgroundColor = MNUtil.hexColorAlpha(
          "#c0bfbf",
          0.8,
        );
      } else {
        ...
        if (!chatAIUtils.isVisionModel(config.model)) {
          let confirm = await MNUtil.confirm(
            "🤖 MN ChatAI",
            `The current model [${config.model}] does not support image input. Continue? \n\n当前模型 [${config.model}] 不支持图片输入，是否继续？`,
          );
          if (!confirm) {
            return;
          }
    ...
    openSetting: function (params) {
      ...
      if (setting.view.hidden) {
        if (setting.isFirst) {
          let width = 330;
          let frame = {
            x: self.view.frame.x - 10,
            y: self.view.frame.y - 20,
            width: width,
            height: 465,
          };
    ...
    changeDynamicFunc: function (button) {
      ...
      try {
        ...
        menu.addMenuItem(
          ...
          selector,
          100,
          currentFunc.length === newOrder.length,
        );
    ...
    setDynamicFunc(param) {
      ...
      try {
        let currentFunc = chatAIConfig.config.dynamicFunc
          ? chatAIConfig.config.dynamicFunc
          : [];
        switch (param) {
          case -1: //None
            currentFunc = [];
            ...
            if (modelName.startsWith(m)) {
              commandTable.push(
                self.tableItem(
                  "👉  " + m,
                  secondeSelector,
                  { source: m, button: button, index: index },
                  true,
                ),
              );
            } else {
    ...
    setPromptModel: function (model) {
      ...
      if (
        !syncDynamicModel &&
        model !== "Default" &&
        !chatAIUtils.checkSubscribe(true)
      ) {
        return;
      }
    ...
    sendButtonTapped: async function () {
      try {
        ...
        if (userInput.trim()) {
          ...
          chatAIConfig.appendDynamicHistory(userInput);
        } else {
    ...
    historyButtonTapped: function (sender) {
      let self = getDynamicController();
      let dynamicHistory = chatAIConfig.getConfig("dynamicHistory");
      let numberOfItems = chatAIUtils.checkSubscribe(false, false, true)
        ? 20
        : 5;
      // MNUtil.showHUD("numberOfItems"+numberOfItems)
      if (dynamicHistory.length > 0) {
        let historyMenu = new Menu(sender, self);
        historyMenu.preferredPosition = 0;
        historyMenu.width = 200;
        dynamicHistory.slice(0, numberOfItems).map((h, index) => {
          historyMenu.addMenuItem("💬  " + h, "replacePrompt:", h);
        });
        historyMenu.show();
      } else {
        self.showHUD("No history");
      }
    ...
    replacePrompt: function (prompt) {
      ...
      // MNUtil.showHUD(prompt)
      self.setInput(prompt);
    ...
    scrollViewDidScroll: function (scrollview) {
      ...
      if (scrollview.id && scrollview.id === "promptScrollView") {
        ...
        self.historyButton.hidden = offset > 30;
      }
...
dynamicController.prototype.init = function () {
  ...
  this.visionImage = MNUtil.getImage(
    chatAIConfig.mainPath + `/vision.png`,
    1.5,
  );
  this.eraserImage = MNUtil.getImage(
    chatAIConfig.mainPath + `/eraser.png`,
    2.1,
  );
  ...
  this.historyButton = this.createButton("historyButtonTapped:");
  this.historyButton.setImageForState(chatAIConfig.historyImage, 0);
  this.historyButton.hidden = true;
  this.historyButton.backgroundColor = MNUtil.hexColorAlpha("#c0bfbf", 0.5);
  // MNButton.addLongPressGesture(this.historyButton, this, "onLongPressHistory:",0.5)
 ...
 * @this {dynamicController}
 */
dynamicController.prototype.setLayout = async function (
  ...
  if (this.miniMode()) {
    ...
    try {
      ...
      this.historyButton.hidden = false;
      ...
      if (chatAIUtils.visionMode) {
        this.visionButton.backgroundColor = MNUtil.hexColorAlpha(
          "#e06c75",
          0.8,
        );
      } else {
      ...
      if (chatAIUtils.noSystemMode) {
        this.dynamicToolButton.backgroundColor = MNUtil.hexColorAlpha(
          "#e06c75",
          0.8,
        );
      } else {
 ...
 * @this {dynamicController}
 */
dynamicController.prototype.refreshVisionAndOCR = async function () {
  let autoOCR = chatAIConfig.getConfig("autoOCR");
  let autoImage = chatAIConfig.getConfig("autoImage");
  if (!autoOCR && !autoImage) {
    chatAIUtils.log(
      "No need to refresh vision and OCR",
      "dynamicController.refreshVisionAndOCR",
    );
    return;
  }
  chatAIUtils.getInfoForDynamic().then((info) => {
    chatAIUtils.visionMode = info.autoImage;
    chatAIUtils.OCREnhancedMode = info.autoOCR;
    // chatAIUtils.log("Refresh vision and OCR", {visionMode:chatAIUtils.visionMode,OCREnhancedMode:chatAIUtils.OCREnhancedMode})
    this.setLayout();
  });
};
/**
 ...
 * @returns {UITextView}
 */
dynamicController.prototype.creatTextView = function (
  superview = "view",
  color = "#e4eeff",
  alpha = 0.9,
) {
  ...
  try {
    ...
    names.map((word, index) => {
      if (!this["nameButton" + index]) {
        this["nameButton" + index] = this.createButton(
          "onPromptButton:",
          "scrollview",
        );
        // this.createButton("nameButton"+index,"onPromptButton:","scrollview")
        this["nameButton" + index].titleLabel.font =
          UIFont.systemFontOfSize(16);
      }
      this["nameButton" + index].hidden = false;
      this["nameButton" + index].setTitleForState(
        chatAIConfig.prompts[word].title,
        0,
      );
      this["nameButton" + index].id = word;
      this["nameButton" + index].backgroundColor = MNUtil.hexColorAlpha(
        "#7093cb",
        0.75,
      );
    ...
    chatAIUtils.addErrorLog(error, "dynamicController.setButtonText", {
      names: names,
      prompts: chatAIConfig.prompts,
    });
  }
};
...
dynamicController.prototype.refreshLayout = function () {
  ...
  this.words.map((word, index) => {
    ...
    let width =
      this["nameButton" + index].sizeThatFits({ width: 100, height: 30 })
        .width + 15;
    ...
    switch (element) {
      ...
      case 2:
        if (
          note.excerptText &&
          note.excerptText !== "" &&
          (!note.excerptPic || note.textFirst)
        ) {
 ...
 * @this {dynamicController}
 */
dynamicController.prototype.getImageForOCR = function (
  noteId,
  userChoice,
  prompt,
) {
 ...
 * @this {dynamicController}
 */
dynamicController.prototype.askWithPromptOnText = async function (
  userInput,
  ocr,
) {
  try {
    ...
    let systemMessage = await chatAIUtils.getTextVarInfo(
      system,
      userInput,
      undefined,
      ocr,
 ...
 * @this {dynamicController}
 */
dynamicController.prototype.askWithPromptOnNote = async function (
  noteid,
  userInput,
  ocr,
) {
  try {
    ...
    let systemMessage = await chatAIUtils.getNoteVarInfo(
      noteid,
      system,
      userInput,
      undefined,
...
dynamicController.prototype.animateTo = async function (targetFrame) {
  ...
  return new Promise((resolve, reject) => {
    ...
    MNUtil.animate(() => {
      ...
      this.historyButton.frame = MNUtil.genFrame(260, 75, 35, 35);
      ...
      this.settingButton.frame = MNUtil.genFrame(
        265,
        this.view.frame.height - 35,
        30,
        30,
      );
...
dynamicController.prototype.setFrame = function (frame) {
  ...
  lastFrame.x = MNUtil.constrain(
    lastFrame.x,
    0,
    studyFrame.width - lastFrame.width,
  );
  lastFrame.y = MNUtil.constrain(
    lastFrame.y,
    0,
    studyFrame.height - lastFrame.height,
  );
 ...
 * @param {*} direction
 */
dynamicController.prototype.popover = function (
  sender,
  commandTable,
  width = 200,
  direction = 2,
) {
  this.popoverController = MNUtil.getPopoverAndPresent(
    sender,
    commandTable,
    width,
    direction,
 ...
 * @returns
 */
dynamicController.prototype.tableItem = function (
  title,
  selector,
  param = "",
  checked = false,
...
dynamicController.prototype.blur = function (delay = 0) {
  MNUtil.delay(delay).then(() => {
    this.runJavaScript(`editor.blur()`);
    this.promptInput.endEditing(true);
    this.promptInput.resignFirstResponder();
  });
};
dynamicController.prototype.setInput = function (prompt) {
  this.runJavaScript(`setInput("${encodeURIComponent(prompt)}")`);
};
    ...
    if (chatAIUtils.visionMode) {
      ...
      if (system.trim()) {
        if (currentNote) {
          ...
          let question = [
            { role: "system", content: systemMessage },
            chatAIUtils.genUserMessage(userInput, imageDatas),
          ];
 ...
 * @param {UIView} view
 */
dynamicController.prototype.showHUD = function (
  title,
  duration = 1.5,
  view = this.view,
) {
